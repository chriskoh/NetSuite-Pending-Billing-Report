function search() 
{
	...
	filters[0]  = new nlobjSearchFilter('type', null, 'is', 'ItemShip');
	...
	filters[4]  = new nlobjSearchFilter('trandate', null, 'onorafter', setStartDate);
	...
	var trandateFile = nlapiCreateFile('IF trandate.txt', 'PLAINTEXT', trandateArray);
	trandateFile.setFolder(363770);
	nlapiSubmitFile(trandateFile);
	
	var typeFile = nlapiCreateFile('IF status.txt', 'PLAINTEXT', statusArray);
	typeFile.setFolder(363770);
	nlapiSubmitFile(typeFile);
	
	var accountFile = nlapiCreateFile('IF tranid.txt', 'PLAINTEXT', tranidArray);
	accountFile.setFolder(363770);
	nlapiSubmitFile(accountFile);
	
	var amountFile = nlapiCreateFile('IF entity.txt', 'PLAINTEXT', entityArray);
	amountFile.setFolder(363770);
	nlapiSubmitFile(amountFile);
	
	var statusrefFile = nlapiCreateFile('IF datecreated.txt', 'PLAINTEXT', datecreatedArray);
	statusrefFile.setFolder(363770);
	nlapiSubmitFile(statusrefFile);
	
	var statusrefFile = nlapiCreateFile('IF amount.txt', 'PLAINTEXT', amountArray);
	statusrefFile.setFolder(363770);
	nlapiSubmitFile(statusrefFile);
	
	var statusrefFile = nlapiCreateFile('IF amountunbilled.txt', 'PLAINTEXT', amounbilledArray);
	statusrefFile.setFolder(363770);
	nlapiSubmitFile(statusrefFile);
	
	var statusrefFile = nlapiCreateFile('IF amountremaining.txt', 'PLAINTEXT', amountremainingArray);
	statusrefFile.setFolder(363770);
	nlapiSubmitFile(statusrefFile);
	
	var statusrefFile = nlapiCreateFile('IF paymentmethod.txt', 'PLAINTEXT', paymentmethodArray);
	statusrefFile.setFolder(363770);
	nlapiSubmitFile(statusrefFile);
	
	var statusrefFile = nlapiCreateFile('IF createdfrom.txt', 'PLAINTEXT', createdfromArray);
	statusrefFile.setFolder(363770);
	nlapiSubmitFile(statusrefFile);
}
